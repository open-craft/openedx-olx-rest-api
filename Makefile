...
# PROJECT_ROOT is usually '/edx/src/openedx-olx-rest-api/' on an Open edX devstack
...
quality: ## check coding style with pycodestyle and pylint
	pylint openedx_olx_rest_api $(PROJECT_ROOT)setup.py
	pylint --py3k openedx_olx_rest_api $(PROJECT_ROOT)setup.py
	pycodestyle $(PROJECT_ROOT)openedx_olx_rest_api $(PROJECT_ROOT)setup.py
	isort --check-only --diff --recursive $(PROJECT_ROOT)openedx_olx_rest_api $(PROJECT_ROOT)setup.py
...
test: clean
	python -m pytest --ds=cms.envs.test --no-cov --nomigrations $(PROJECT_ROOT)openedx_olx_rest_api/tests/
...
extract_translations: ## extract strings to be translated, outputting .mo files
	rm -rf docs/_build
	cd openedx-olx-rest-api && ../manage.py makemessages -l en -v1 -d django
	cd openedx-olx-rest-api && ../manage.py makemessages -l en -v1 -d djangojs

compile_translations: ## compile translation files, outputting .po files for each supported language
	cd openedx-olx-rest-api && ../manage.py compilemessages

detect_changed_source_translations:
	cd openedx-olx-rest-api && i18n_tool changed
...
dummy_translations: ## generate dummy translation (.po) files
	cd openedx_olx_rest_api && i18n_tool dummy
